...
router.get('/', async (req, res) => {
  ...
  const [rows] = await pool.query(
   `SELECT w.id, w.corp_code, w.alias, w.priority, w.created_at,
       ...
       FROM DL_WISHLIST w
       JOIN DL_CORP_BASIC cb ON cb.corp_code = w.corp_code
      WHERE w.user_id=?
...
router.post('/', async (req, res) => {
  ...
  await pool.query(
    `INSERT INTO DL_WISHLIST (user_id, corp_code, created_at)
     VALUES (?,?,NOW())
...
router.delete('/:corp_code', async (req, res) => {
  ...
  await pool.query(`DELETE FROM DL_WISHLIST WHERE user_id=? AND corp_code=?`, [uid, cc]);
  res.json({ ok: true });
});
