...
프로젝트의 상세한 설계 및 구현 사항은 다음 문서를 참고하세요:

### 백엔드 설계 문서

- **[BN_WAS_설계서_v2.md](BN_WAS_설계서_v2.md)**
  - 백엔드 전체 아키텍처 및 프로세스
  - API 호출 흐름 및 인증/권한 처리
  - 공통 결과코드표 (retCode 10~99)
  - 전체 URL 목록 (WEB API + PC APP)
  - 환경변수 설정 가이드

### API 사용자 가이드

- **[BN_WAS_API_사용자설계서_v2.md](BN_WAS_API_사용자설계서_v2.md)**
  - 모든 API 엔드포인트 상세 명세
  - 요청/응답 파라미터 및 예시
  - 인증 방식 및 오류 코드
  - Postman/curl 예제
...
- **[DB정의서_V4.md](DB정의서_V4.md)**
  - 13개 활성 테이블 정의
  - 컬럼 상세 명세 (Type, Length, Null, PK, FK)
  - 인덱스 정의 및 최적화 전략
  - 테이블명 변경 이력 (DL_ 접두사 통합)
  - 설계 규칙 및 용어 사전
